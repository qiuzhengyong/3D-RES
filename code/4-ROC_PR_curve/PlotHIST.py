...
def Plot_HIST_Fn(label,distance, save_path, num_bins = 50):
    ...
    fig = plt.figure()
    plt.hist(gen_dissimilarity_original, bins, alpha=0.5, facecolor='blue', density=False,  label='gen_dist_original')
    plt.hist(imp_dissimilarity_original, bins, alpha=0.5, facecolor='red', density=False, label='imp_dist_original')
...
if __name__ == '__main__':
    ...
    label = np.load(os.path.join(FLAGS.plot_dir,'target_label_vector.npy'))
